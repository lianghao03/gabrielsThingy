...
use \Exception;
use \PDO;
use HkDB\UserQuery as ChildUserQuery;
use HkDB\Map\UserTableMap;
 ...
 *
 *
 * @package    propel.generator.HkDB.Base
 */
abstract class User implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\HkDB\\Map\\UserTableMap';
    ...
    /**
     * The value for the skills field.
     *
     * @var        array
     */
    protected $skills;
    ...
    /**
     * The unserialized $skills value - i.e. the persisted object.
    ...
    /**
     * Initializes internal state of HkDB\Base\User object.
     */
    ...
    /**
     * Get the [skills] column value.
     *
     * @return array
     */
    public function getSkills()
    {
        if (null === $this->skills_unserialized) {
            $this->skills_unserialized = array();
        }
        if (!$this->skills_unserialized && null !== $this->skills) {
            $skills_unserialized = substr($this->skills, 2, -2);
            $this->skills_unserialized = '' !== $skills_unserialized ? explode(' | ', $skills_unserialized) : array();
        }

        return $this->skills_unserialized;
    }

    /**
     * Test the presence of a value in the [skills] array column value.
     * @param      mixed $value
     *
     * @return boolean
     */
    public function hasSkill($value)
    {
        return in_array($value, $this->getSkills());
    } // hasSkill()
    ...
    /**
     * Set the value of [skills] column.
     *
     * @param array $v new value
     * @return $this|\HkDB\User The current object (for fluent API support)
     */
    public function setSkills($v)
    {
        if ($this->skills_unserialized !== $v) {
            $this->skills_unserialized = $v;
            $this->skills = '| ' . implode(' | ', $v) . ' |';
            $this->modifiedColumns[UserTableMap::COL_SKILLS] = true;
        }
    ...
    /**
     * Adds a value to the [skills] array column value.
     * @param  mixed $value
     *
     * @return $this|\HkDB\User The current object (for fluent API support)
     */
    ...
    {
        $currentArray = $this->getSkills();
        $currentArray []= $value;
        $this->setSkills($currentArray);

        return $this;
    } // addSkill()

    /**
     * Removes a value from the [skills] array column value.
     * @param  mixed $value
     *
     * @return $this|\HkDB\User The current object (for fluent API support)
     */
    public function removeSkill($value)
    {
        $targetArray = array();
        foreach ($this->getSkills() as $element) {
            if ($element != $value) {
                $targetArray []= $element;
            }
        }
        $this->setSkills($targetArray);
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\HkDB\User The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \HkDB\User (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \HkDB\User Clone of current object.
     * @throws PropelException
     */
