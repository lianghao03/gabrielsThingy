...
abstract class UserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \HkDB\Base\UserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\HkDB\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, username, password, skills FROM users WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the skills column
     *
     * @param     array $skills The values to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function filterBySkills($skills = null, $comparison = null)
    {
        $key = $this->getAliasedColName(UserTableMap::COL_SKILLS);
        ...
        return $this->addUsingAlias(UserTableMap::COL_SKILLS, $skills, $comparison);
    }

    /**
     * Filter the query on the skills column
     * @param     mixed $skills The value to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::CONTAINS_ALL
     *
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function filterBySkill($skills = null, $comparison = null)
    {
        if (null === $comparison || $comparison == Criteria::CONTAINS_ALL) {
            if (is_scalar($skills)) {
                $skills = '%| ' . $skills . ' |%';
            ...
            $key = $this->getAliasedColName(UserTableMap::COL_SKILLS);
            if ($this->containsKey($key)) {
                $this->addAnd($key, $skills, $comparison);
            } else {
                $this->addAnd($key, $skills, $comparison);
            }
            $this->addOr($key, null, Criteria::ISNULL);
